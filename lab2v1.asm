...
Inc_Done:
	...
	mov a, Seconds
	cjne a,#0x59,sec_func
	mov a,minutes
	cjne a,#0x59,min_func
	mov a, hours
	cjne a,#0x12,hrs_func
	cpl AMPMSEL
	mov a,#0x01	
	ljmp K6
	K7:
...
min_func:	
	jnb UPDOWN, Timer2_ISR_decrement1
	add a, #0x01	
	K4:	
	sjmp Timer2_ISR_da1
	Timer2_ISR_decrement1:
	add a, #0x99 ; Adding the 10-complement of -1 is like subtracting 1.
	Timer2_ISR_da1:
	da a ; Decimal adjust instruction.  Check datasheet for more details!
	ljmp K5
...
hrs_func:	
	...
	jnb UPDOWN, Timer2_ISR_decrement2
	add a, #0x01
	K6:	
	sjmp Timer2_ISR_da2
	Timer2_ISR_decrement2:
	add a, #0x99 ; Adding the 10-complement of -1 is like subtracting 1.
	Timer2_ISR_da2:
	da a ; Decimal adjust instruction.  Check datasheet for more details!
	ljmp K7
...
main:
	...
	mov minutes, #0x00
	mov hours, #0x01
	mov alarm_hours, #0x05
	mov alarm_minutes, #0x00
	; After initialization the program stays in this 'forever' loop


loop:
	jb BOOT_BUTTON, hour_button_check  ; if the 'BOOT' button is not pressed skip
...
hour_button_check:
	jb clock_hours, min_button_check
...
min_button_check:
	jb clock_min, alarm_min_button_check
	Wait_Milli_Seconds(#50)
	jb clock_min, alarm_min_button_check
	jnb clock_min,$
	ljmp mins_inc		

alarm_min_button_check:
	jb alarm_min, alarm_hrs_button_check
	Wait_Milli_Seconds(#50)
	jb alarm_min, alarm_hrs_button_check
	jnb alarm_min,$
	ljmp alarm_mins_inc		

alarm_hrs_button_check:
	jb alarm_hrs, loop_a
	Wait_Milli_Seconds(#50)
	jb alarm_hrs, loop_a
	jnb alarm_hrs,$
...
loop_b:
    ...
    clr TR0
    mov a, hours
    cjne a, alarm_hours, C1
    mov a, minutes
    cjne a, alarm_minutes, C1
    ;mov a, AMPMSEL
    ;cjne a, AMPMSEL1, C1
    setb TR0
